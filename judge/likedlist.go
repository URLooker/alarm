...
func (this *SafeLinkedList) HistoryData(limit int) ([]*HistoryData, bool) {
	...
	firstItem := firstElement.Value.(*model.ItemStatus)
	...
	for i < limit {
		...
		vs[i] = &HistoryData{
			Timestamp: nextElement.Value.(*model.ItemStatus).PushTime,
			Value:     nextElement.Value.(*model.ItemStatus).Result,
		}
		i++
...
func (this *SafeLinkedList) PushFrontAndMaintain(v *model.ItemStatus, maxCount int) bool {
	this.Lock()
	defer this.Unlock()
	...
	if sz > 0 {
		...
		if v.PushTime <= this.L.Front().Value.(*model.ItemStatus).PushTime || v.PushTime <= 0 {
			return false
		}
	}
